...
The Markdown files in the `md` directory might be overwritten in the future, do not change any files there. Instead, here's how to make changes:

- To edit individual remarks in the original, create an override in `overrides/`.
- To edit a translation, use the translation tool or manually fix it in `md-en/`.
- To edit any other source files (e.g. CSS, site structure), edit the files directly.

Commit your changes **on a new branch** (not `main`) and push the branch. The content will be deployed to [staging.wittgensteinnachlass.com](https://staging.wittgensteinnachlass.com/). Once you are happy with the change and want to push it to production, open a Pull Request and merge it from your branch into `main`, which will deploy it to [wittgensteinnachlass.com](https://www.wittgensteinnachlass.com/).
...
The remaining published assets are **generated from these sources and are not committed** — they are regenerated by the build (locally via `make production`, or in CI on every push). This includes `viz/` (SVG visualizations), `covers/` (SVG book covers), `epub/`, `pdf/`, `graphics-cropped/`, and the Hugo output under `site/`. English translations in `md-en/` are also generated (by the translate tool) but, unlike the above, are committed as source so the site can be built without running an LLM. See [Building](#building).

## Building

The `Makefile` orchestrates the build pipeline. All commands assume the markdown source files in `md/` already exist.

The site is built for one of two Hugo environments: **production** hides the not-yet-public English translations (the per-document German/English links and the search language switcher), while **staging** keeps them visible for review. Pushing to `main` deploys production; pushing any other branch deploys to a shared staging zone.

- **`make production`** / **`make staging`** -- full build from scratch: generates visualizations, covers, ebooks, PDFs, then builds the Hugo site for that environment. Run this after a fresh checkout or when the source markdown changed.
- **`make quick`** -- rebuilds only the Hugo content and site for staging (skips covers, ebooks, PDFs, visualizations; assumes they already exist). Useful for iterating on site templates or CSS. Add `ENV=production` (`make quick ENV=production`) to re-render the production layouts (translations hidden) without regenerating assets.
- **`make serve`** -- live local preview with auto-reload, using the staging environment (translations visible). Add `ENV=production` (`make serve ENV=production`) to preview the production layouts (translations hidden).
- **`make deploy`** -- rebuilds Hugo content, runs a production Hugo build, and deploys via `tools/deploy_site.sh`. Does _not_ regenerate assets -- run `make production` first if covers, ebooks, or PDFs changed.
- **`make production deploy`** -- full rebuild followed by deployment.
...
Mathematical expressions are rendered as inline MathML, not LaTeX. Series numbers and section markers use `<span class="series-number">` elements. The following Markdown constructs are used:
